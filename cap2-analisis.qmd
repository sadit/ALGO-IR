...
Todos los problemas entonces se deben plantear en forma de lenguajes, i.e., cadenas de símbolos; esto es, en términos de _dada la cadena S ¿la máquina $M$ la acepta?_, dónde _acepta_ quiere decir que la máquina es capaz de ir desde el estado inicial al estado de aceptación leyendo $S$. Si $M$ no termina en un estado de aceptación, entonces implicará un _no_ (e.g., no existe una definición en la función de transición para un estado y símbolo o no es suficiente para llegar a un estado de aceptación).

Por ejemplo, sea $M_{01}$ una máquina capaz de detectar $n \geq 0$ ceros terminados por un único $1$. Entonces

$$M_{01} = (\{q_0, q_1, q_2\}, \{0, 1\}, \{0, 1, X, \epsilon \}, q_0, \epsilon, \{q_2\}, \delta_{01})$$

donde la función de transición se define como:

- $\delta_{01}(q_0, 0) = (q_0, X, R)$
- $\delta_{01}(q_0, 1) = (q_1, X, R)$
- $\delta_{01}(q_1, \epsilon) = (q_2, \epsilon, R)$
...
$$(\{q_0, q_1, q_2, q_3, q_4\}, \{0, 1\}, \{0, 1, X, Y, \epsilon\}, q_0, \epsilon, \{q_4\}, \delta),$$
...
- $\delta(q_2, Y) = (q_2, Y, L)$
- $\delta(q_3, Y) = (q_3, Y, R)$
- $\delta(q_3, \epsilon) = (q_4, \epsilon, R)$
...
::: {.callout-important}
Las máquinas de Turing son expresiones mínimas de lo que es cómputable con una clara analogía mecánica, lo cual hace evidente su implementación en el mundo físico. 
:::
